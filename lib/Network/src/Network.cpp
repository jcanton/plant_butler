...
NetworkClient::NetworkClient(const char *ssid, const char *pass)
...
void NetworkClient::connectWiFi() {
  ...
  Serial.println("WiFi firmware version: " + String(WiFi.firmwareVersion()) +
                 " / " + String(WIFI_FIRMWARE_LATEST_VERSION));
...
bool NetworkClient::connectedToServer() { return client.connected(); }
...
void NetworkClient::sendData(std::map<std::string, float> data) {
  ...
  client.println("GET  /test/arduino_landing.php" + String(query_string) +
                 " HTTP/1.1");
